...
p1<- ggplot() +
      # blue plot
      geom_point(data=mtcars[mtcars$cyl==8,], aes(x=wt, y=mpg)) + 
      geom_smooth(data=mtcars[mtcars$cyl==8,], aes(x=wt, y=mpg), fill="blue",
    ...
    p2<- ggplot() +      
      # red plot
      geom_point(data=mtcars[mtcars$cyl==8,], aes(x=wt, y=hp)) + 
      geom_smooth(data=mtcars[mtcars$cyl==8,], aes(x=wt, y=hp), fill="red",
                  colour="red", size=1,method="lm")+
      labs(x="Weight in 1000 lbs",y="Gross horsepower") 
    
    grid.arrange(p1, p2, ncol = 2, 
                       main = "Miles/US Gallon & HorsePower Vs Weight for 8 cylinder")
